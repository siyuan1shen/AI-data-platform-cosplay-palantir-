...
export function PublishedProjectGate({ projectId, children }: PropsWithChildren<{ projectId: string }>) {
  ...
  if (publicationsQuery.isLoading) {
    return <StatusMessage title="正在检查企业投影版本" description="使用端只读取已经正式发布的投影。" />;
  }
  ...
  if (!publicationsQuery.data?.total) {
    return <StatusMessage title="当前项目还没有正式发布版本" description="请先在建设端完成企业投影审核并发布；使用端不会直接操作未发布草稿。" />;
  }
  return <>{children}</>;
}
